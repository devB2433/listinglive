...
class QuotaPackagePlan(Base):
    __tablename__ = "quota_package_plans"

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    package_type: Mapped[str] = mapped_column(String(32), unique=True, nullable=False, index=True)
...
class QuotaPackage(Base):
    ...
    quota_package_plan_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("quota_package_plans.id"),
        nullable=True,
    )
